...
미니맵은100×300px(모바일64×220px)이다. 마우스를 올리거나 터치하는 동안 탐색하며 이탈·해제 시 자동 추적으로 복귀한다. 방향키·Home·End 탐색과 Escape·포커스 이탈 복귀도 지원한다. 준비·일시정지·종료에서도 사용할 수 있고 결승 배속·판정은 계속 유지된다. 순위는 최소200px, 최대3열 카드이며 보이는 행만 그린다. 검색·카드 추적·전체 결과 복사는 유지한다.
