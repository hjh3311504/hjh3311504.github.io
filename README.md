...
PR에서는 `site-build`가 정적 파일을1회 만들고, `checks`가 lint·정적 검사·전체 단위 테스트를 병렬로 실행합니다. 브라우저 검사는 같은 build 파일을 내려받습니다. 성능·시간 보존 검사는 전용 실행 환경에서 실행하고, 일반 브라우저 검사는4개 환경에 나눕니다. 각 환경 안에서는 기존처럼 검사1개씩 실행합니다.

일반 검사의 배치는 `node scripts/plan_browser_shards.js`로 만듭니다. 이전 PR의 실행 시간을 기준으로 긴 파일과 실제 경기 종료 검사4개를 분산합니다. 새 검사는 자동으로 포함하며, 생성한 목록을 Playwright로 다시 읽어 누락·중복을 검사합니다. 보고서는 `browser-1`~`browser-4`에 저장합니다. 측정 시간은 배치용이며 테스트 조건·제한 시간·재시도 횟수는 유지합니다.

`build`는 모든 필수 작업의 성공을 확인하는 마지막 검사입니다. 실패·취소·예상 밖 생략이 있으면 실패합니다. 변경 이유와 측정 기준은 [PR 브라우저 검사 분산 결정](docs/adr/2026-09-27-PR-브라우저-검사-분산.md)을 참고하세요.
