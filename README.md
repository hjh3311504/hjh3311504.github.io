...
`node scripts/measure_marble_balance.js`는3개 맵×2/10/20/30/60/100개×난수10개의180경기를 비교한다. `--large`는300/1000개를 각 맵에서600경기초까지 관측한다. 첫번째·여러명3명·n번째(중간 순번)·마지막을 기록한다. 배속을 반영한 계산상 재생 시간과 CPU 실행 시간을 구분하며 실제 브라우저 시간을 대신하지 않는다. 최신 결과와 남은 한계는 [화면·경기 길이·음향 개선 ADR](docs/adr/2026-09-18-화면-경기길이-음향-개선.md)을 따른다.
