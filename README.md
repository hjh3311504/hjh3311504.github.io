...
도감 제목은 도감으로 표시하고 기본 블록·특수 블록 아래에 스킬 영역을 둔다. 원형 파동은 같은 카드·그리드·경기 그림과 소리 미리듣기를 제공한다. 파동 발동 순간에만 짧은 훙 효과음을 한 번 내며 표시 중 반복하지 않는다. 스킬 ON일 때 경기 준비·재개·음소거 해제에서 음원을 준비한다. 미리듣기와 경기에서 pulse-whoosh-deep-v2.wav를 공유하며 음정·속도는 배속과 무관하다. 화면 범위와 바깥52의 감쇠·음소거·볼륨·정지 정리를 따른다. 파동은 동시1개·최소200ms로 제한하고 일반 충돌음보다 먼저 고른다. 전체6개가 차면 팡파레를 제외한 기존 소리를 짧게 줄여 교체하며 전체 최소28ms 간격을 지킨다.
